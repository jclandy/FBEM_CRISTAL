...
sigma_s = 0.002; % snow rms height (default = 0.001 m)
l_s = 0.02; % snow correlation length (default = 0.04 m)
T_s = -15; % snow bulk temperature (default = -20 C)
rho_s = 300; % snow bulk density (default = 350 kg/m^3)
r_s = 0.003; % snow grain size (normal range from 0.0001 to 0.004 m, default 1 mm)
h_s = 0.1; % snow depth, m

sigma_si = 0.002; % sea ice rms height (default = 0.002 m)
l_si = 0.02; % sea ice correlation length (default = 0.02 m)
T_si = -5; % sea ice bulk temperature (default = -15 C)
...
prf = 15119; % pulse-repetition frequency (default = 18182 Hz, e.g. Cryosat-2)
bandwidth = 500*10^6; % antenna bandwidth (default = 320*10^6 Hz, e.g. Cryosat-2)
% G_0 = 42.2; % peak antenna gain, dB, ku-band
...
sigma_surf_s = 0.1; % large-scale snow roughness height (default = 0.1 m)
sigma_surf_si = 0.05; % large-scale sea ice roughness height (default = 0.1 m)
l_surf = 5; % large-scale correlation length (default = 5 m)
H_surf = 0.5; % Hurst parameter (default = 0.5)
dx = 5; % resolution of grid, m (WARNING use dx>=10 for PL mode and dx>=5 for SAR mode)
...
% Optional Plotting
topo_plot = 0; % example plot of tetrahedral surface mesh
    ...
    for j = 1:length(vec2)
        ...
        for k = 1:length(vec3)
            ...
            for l = 1:itN % Average over n iterations
                ...
                [PosT_s,PosT_si,surface_type] = synthetic_topo_shell(op_mode,topo_type,pitch,roll,sigma_surf_s,sigma_surf_si,l_surf,H_surf,dx,L_w,L_h,D_off,f_p);
                
                % Run Facet-Based Echo Model
                [P_t_full(:,:,l),P_t_ml(:,l),P_t_full_comp(:,:,:,l),P_t_ml_comp(:,:,l)] = Facet_Echo_Model(op_mode,lambda,bandwidth,P_T,h,v,pitch,roll,prf,beam_weighting,G_0,D_0,gamma1,gamma2,N_b,t,PosT_s,PosT_si,surface_type,sigma_0_snow_surf,sigma_0_snow_vol,kappa_e,tau_snow,c_s,h_s,sigma_0_ice_surf,sigma_0_lead_surf,sigma_0_mp_surf);
            ...
            if (topo_plot | echo_plot)>0
                Plotting(topo_plot,echo_plot,PosT_si,t,P_t_ml_range{i,j,k},P_t_full_range{i,j,k},P_t_ml_comp_range{i,j,k},N_b,epsilon_b);
            else
            end
